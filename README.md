...
安装顺序不影响 Codex 行为。项目执行使用依赖关系而不是死板轮次。客户范围、设计方向、生产发布/流量切换、付费激活、费用和破坏性操作需要明确确认；用户批准一个有边界的前端/后端/最小测试工作包后，包含的执行门禁通过即可连续完成，不再逐阶段重复确认：
...
- 当前步骤不在已批准工作包内时才停止确认；工作包内的执行步骤验证通过后可以自动继续。
