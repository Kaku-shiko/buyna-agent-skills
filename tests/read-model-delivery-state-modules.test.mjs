...
test("manifest and complete installer expose both fixed modules exactly once", async () => {
...
test("runtime outputs are exact, immutable, scoped, bounded, and contact-safe", async () => {
  ...
  const core = deliveryApi.createDeliveryStateCore({
    ...
    deliveryIdGenerator: () => "delivery_1", attemptIdGenerator: () => "attempt_1",
  });
  ...
  const serialized = JSON.stringify({ overview, sourceEvent, pending, delivered });
  for (const forbidden of [
    "password", "token", "cookie", "card", "cvv", "secret", "rawProviderResponse",
    "stack", "buyer@example", "+8190", "private subject", "private body",
