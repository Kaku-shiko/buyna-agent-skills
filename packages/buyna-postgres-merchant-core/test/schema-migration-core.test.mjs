import test from 'node:test';import assert from 'node:assert/strict';import{assertCutoverEvidence,assertOwnedRows,validateExistingSchemaMigration}from'../src/schema-migration-core.mjs';
const plan={resourceMode:'existing_buyna_resources',projectId:'shop-a',sellerId:'shop-a',database:'buyna_merchants',sourceSchema:'public',targetSchema:'shop_a',targetDatabase:'buyna_merchants',newEc2Instances:0,newDatabases:0,newBuckets:0,newPorts:0,allowCreateRds:false,allowCreateDatabase:false,approval:'approved_reversible_migration'};
test('accepts an approved same-database schema migration',()=>assert.equal(validateExistingSchemaMigration(plan).searchPath,'shop_a,public'));
