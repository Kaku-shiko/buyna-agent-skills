...
test('product merchant navigation contains exactly the approved six pages in order',()=>{
  assert.deepEqual(DASHBOARD_NAVIGATION.map(item=>[item.key,item.label,item.href]),[
    ['dashboard','仪表盘','/seller'],
    ['products','商品管理','/seller/products'],
    ['categories','分类管理','/seller/categories'],
    ['orders','订单','/seller/orders'],
    ['paidCustomers','付费客户','/seller/paid-customers'],
    ['paymentSettings','支付设置','/seller/settings/payment'],
  ]);
});
...
test('each fixed page declares the controls and states proven useful in previous cases',()=>{
  assert.deepEqual(DASHBOARD_PAGES.dashboard.metrics,['activeProducts','pendingOrders','paidOrders','paidCustomers']);
  assert.deepEqual(DASHBOARD_PAGES.products.actions,['create','edit','archive','visibility','images','inventory','reorder']);
  assert.deepEqual(DASHBOARD_PAGES.orders.filters,['search','status','month']);
  assert.deepEqual(DASHBOARD_PAGES.orders.actions,['detail','refreshPayments','exportCsv']);
  assert.deepEqual(DASHBOARD_PAGES.paidCustomers.actions,['detail','exportCsv','contact']);
