export const DASHBOARD_NAVIGATION=Object.freeze([
  ...
  {key:'coupons',label:'优惠券管理',href:'/seller/coupons',icon:'券'},
...
export const DASHBOARD_TABLES=Object.freeze({
  ...
  coupons:{columns:[column('name','优惠券'),column('couponType','类型'),column('ruleSummary','优惠规则'),column('period','有效期'),column('status','状态'),column('usage','已使用 / 已预约 / 剩余',{align:'right'}),column('actions','操作',{align:'right'})]},
  orders:{columns:[column('orderNumber','订单号'),column('customer','客户'),column('amount','金额',{align:'right'}),column('discount','优惠',{align:'right'}),column('paymentMethod','支付方式'),column('status','状态'),column('createdAt','时间'),column('actions','操作',{align:'right'})]},
...
export const COUPON_FORM_FIELDS=Object.freeze({
  common:['name','couponType','startsAt','endsAt','totalLimit','perCustomerLimit'],
  percentage_code:['code','percentageBps','minimumSubtotal','maximumDiscount'],
  fixed_threshold:['discountAmount','thresholdQuantity','thresholdAmount','thresholdOperator','applicationMode'],
});
export const COUPON_LIFECYCLE=Object.freeze({
  draft:['active','archived'],active:['paused','expired','archived'],paused:['active','expired','archived'],expired:['archived'],archived:[],
});
export function couponFormFields(type){
  if(!Object.hasOwn(COUPON_FORM_FIELDS,type))throw new Error('INVALID_COUPON_TYPE');
  return[...COUPON_FORM_FIELDS.common,...COUPON_FORM_FIELDS[type]];
}
...
export const DASHBOARD_PAGES=Object.freeze({
  ...
  coupons:{table:'coupons',filters:['search','status','couponType'],actions:['create','edit','saveDraft','activate','pause','reactivate','archive'],states:[...commonStates,'validation','success']},
