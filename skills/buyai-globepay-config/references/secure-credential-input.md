...
- 用户确认已提供当前商家、当前环境配套的 Partner Code 和 Credential Code，
  或服务端确认两者已保存时，按商家已开通渠道处理；不再要求重复开通。
  用户确认属于开通依据，不冒充渠道接口核验记录。
- 尚未保存凭据或接上网站服务端：显示“待完成支付配置”。
- 尚未验证签名、下单、通知/查询：显示“支付接入待验证”。真实交易未验收单独说明。
- 本地收款开关关闭：说明“网站收款尚未启用”，不能推断支付服务商未开通渠道。
- 只有可信渠道接口明确返回该渠道未开通/无权限时，才报告对应开通或权限问题。
  无效 Code、签名错误、网络超时、缺少本地订单必须按实际错误解释。
- 普通售卖页面默认正常购买/支付文案；开发进度放在交付说明。操作确实不可用时
  显示真实状态，但不得将未验证改写成未开通，也不生成虚假的支付成功。

以下保留的是 edu 的安全保存/验证顺序；其本地开关状态不代表渠道侧开通状态。
...
建议状态分别保存：`channelStatus`（开通状态及商家确认/接口依据）、`storageStatus`（未配置/已保存）、`validationStatus`（待验证/通过/失败/过期）、`collectionEnabled`（网站收款开关）。缺少交易验证不会将已确认的渠道开通状态改为未开通；配套凭据已提供时不重复要求开通。开通也不代表网站已接通或已有成功交易。凭据轮换后重新验证当前版本，不自动推断渠道未开通。
