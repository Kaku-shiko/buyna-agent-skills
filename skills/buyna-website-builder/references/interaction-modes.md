...
- Work only on the current gate or the remaining gates in the approved work package.
...
- Both modes require the same delivery evidence. Explicit approval is required at decision points; approved execution gates use `completeAuthorizedGate` without another confirmation.
...
| approval of a named bounded execution package | `authorizeWorkPackage`; subsequent included gates use `completeAuthorizedGate` |
