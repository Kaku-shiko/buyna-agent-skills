...
- Step 5: immediately call `buyai-dashboard-data-interaction`. It begins the approved Dashboard's data-interaction sequence by routing only the current delivery slice to `buyna-project-framework`. Require both an executable server/API foundation and real frontend contract/client/adapter/state code with passing frontend checks; backend-only delivery fails this step.
- Step 6: continue `buyai-dashboard-data-interaction`; it routes the current data/storage slice to `buyna-aws-data-layer` and/or `buyna-s3-storage`.
