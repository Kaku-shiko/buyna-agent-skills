...
export function AIShoppingGuideInline() {
  ...
  async function send(text: string) {
    ...
    try {
      ...
      const resp = await fetch("/api/ai-shopping-guide", {
        ...
        body: JSON.stringify({ session_id: sessionIdRef.current, message: t, history }),
      });
    ...
    <div className="glass overflow-hidden rounded-3xl border border-border">
      <div className="flex items-center justify-between border-b border-border px-5 py-4">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg btn-primary">
      ...
      <div
        ref={scrollRef}
        className="max-h-[420px] min-h-[280px] space-y-3 overflow-y-auto px-4 py-4"
      >
        {messages.map((m, i) => (
      ...
      <div className="border-t border-border px-4 py-3">
        <div className="mb-2 flex flex-wrap gap-2">
          {copy.suggestions.map((s) => (
            <button
              ...
              onClick={() => void send(s)}
              disabled={loading}
              className="rounded-full border border-border bg-background/50 px-3 py-1 text-xs text-muted-foreground transition-colors hover:border-primary/60 hover:text-foreground disabled:opacity-50"
            >
              {s}
            </button>
          ))}
        ...
        >
          <textarea
            ...
            className="min-h-[42px] max-h-32 flex-1 resize-none rounded-lg border border-border bg-background/60 px-3 py-2 text-sm outline-none focus:border-primary"
          />
          <button
            type="submit"
